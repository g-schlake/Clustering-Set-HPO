...
class InterestingnessScorerOptunity(InterestingnessScorer):
    ...
    def __init__(self, validation_functions, algorithms, data, similarity_function, interestingness_function, method=generationMethod.BINARY, time_budget=20):#, num_algs=10):
            ...
            for alg, number in algorithms:
                ...
                for i in range(number+1):
                    search[alg.name][str(i)]={
                        f"{alg.name}_{i}_{key}_{j}":value for key, value in self.algorithm_dict[alg.name][0].hyperparams.items() for j in range(i)
                    }
                    if i ==0:
    ...
    def process_hparams(self, **kwargs):
        if self.method==generationMethod.FIXED:
            ...
            raise ValueError
        hyperparams = [{key: value for key, value in dict(kwargs).items() if int(key[-1])==i} for i in range(self.num_algs)]
