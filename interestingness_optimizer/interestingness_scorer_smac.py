...
from auxiliaries.misc import finite_number, generationMethod
from auxiliaries.decorators import timed
from interestingness_optimizer.interestingness_scorer import InterestingnessScorer, generate_validation, \
    generate_clustering, generate_similarity
from interestingness_functions import naive_interestingness_avg, naive_interestingness_mean_sim, naive_interestingness_min_sim
...
class InterestingnessScorerSMACBase(InterestingnessScorer):
    ...
    def __init__(self, validation_function, algorithms, data, similarity_function, interestingness_function, method=1, time_budget=20):
                ...
                for i in range(number):
                    ...
                    for key, value in self.algorithm_dict[alg.name][0].hyperparams.items():
                        ...
                        if key in alg.integer_params:
                            ...
                            params[param_name]=Float(param_name, (alg.hyperparams[key][0], alg.hyperparams[key][1]))
                    conditions.append(EqualsCondition(child=params[param_name], parent=params[f"{alg.name}_{i}"], value=True))
                ...
                for i in range(number):
                    for key, value in self.algorithm_dict[alg.name][0].hyperparams.items():
                        ...
                        if key in alg.integer_params:
                            ...
                            params[param_name]=Float(param_name, (alg.hyperparams[key][0], alg.hyperparams[key][1]))
                        conditions.append(GreaterThanCondition(child=params[param_name],parent=params[alg.name], value=i))
...
class InterestingnessScorerSMAC_HPO(InterestingnessScorerSMACBase):
    def __init__(self, validation_function, algorithms, data, similarity_function, interestingness_function, method=1,time_budget=20):
        super().__init__(validation_function, algorithms, data, similarity_function, interestingness_function, method, time_budget)
