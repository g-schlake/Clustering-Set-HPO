...
class InterestingnessScorerBOHB(InterestingnessScorer):
    ...
    def __init__(self, validation_functions, algorithms,data, similarity_function, interestingness_function, method=generationMethod.BINARY, time_budget=20):
        ...
        if method==generationMethod.FIXED:
            ...
            for alg, alg_num in self.algorithms:
                for i in range(alg_num):
                    ...
                    for key, param in hyperparams.items():
                        ...
                        if key in alg.integer_params:
                            cs_list.append((present:=cs.IntegerUniformHyperparameter(child_name, param[0], param[1],parent==True)))
                        else:
                            cs_list.append((present:=cs.UniformHyperparameter(child_name, param[0], param[1],parent==True)))
            ...
            for alg, alg_num in self.algorithms:
                ...
                for i in range(alg_num):
                    ...
                    for key, param in hyperparams.items():
                        if key in alg.integer_params:
                            cs_list.append((present:=cs.IntegerUniformHyperparameter(f"{alg.name}_{i}_{key}", param[0], param[1],parent>i)))
                        else:
                            cs_list.append((present:=cs.UniformHyperparameter(f"{alg.name}_{i}_{key}", param[0], param[1], parent>i)))
